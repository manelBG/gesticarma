  ...
  useEffect(() => {
    ...
    const fetchMissions = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/missions/employee/${user._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  return (
    <div className="p-8 space-y-11">
      <div className="bg-white rounded-3xl shadow-lg p-10 space-y-10 max-w-7xl mx-auto">
        ...
        <p className="text-gray-700 text-lg font-medium">Voici un aperçu de vos missions </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <DashboardCard icon={ClipboardList} label="Missions" value={missions} to="/missions" />          
