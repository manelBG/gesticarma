...
import { Car, Users, Wrench, ClipboardList, Calendar, Settings2 } from 'lucide-react';
...
export default function AdminAccueil() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      setChartData([
        { mois: 'Jan', missions: 12, interventionsInternes: 5, interventionsExternes: 3 },
        { mois: 'Fév', missions: 18, interventionsInternes: 6, interventionsExternes: 4 },
        { mois: 'Mar', missions: 25, interventionsInternes: 10, interventionsExternes: 5 },
        { mois: 'Avr', missions: 20, interventionsInternes: 11, interventionsExternes: 7 }
      ]);
  ...
  return (
    <div className="p-8 space-y-11">
      <div className="bg-white rounded-3xl shadow-lg p-10 space-y-10 max-w-7xl mx-auto">
        ...
        <p className="text-gray-700 text-lg font-medium">Voici un aperçu global de l'activité du parc automobile</p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <DashboardCard icon={Users} label="Employés" value={stats?.totalEmployees || 0} to="/employes" />
          ...
          <DashboardCard icon={Users} label="Techniciens" value={stats?.totalTechniciens || 0} to="/techniciens" />
          <DashboardCard icon={Settings2} label="Interventions Internes" value={stats?.totalInterventions || 0} to="/interventions/interne" />
          <DashboardCard icon={Wrench} label="Interventions Externes" value={stats?.totalInterventionsExternes || 0} to="/interventions/externe" />
          
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gray-50 rounded-xl shadow-inner p-4">
            <h3 className="text-lg font-semibold text-blue-900 mb-4">Activité mensuelle : Missions & Interventions</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData}>
                <XAxis dataKey="mois" />
                <YAxis />
                <Tooltip />
                <Bar dataKey="missions" fill="#2563eb" radius={[10, 10, 0, 0]} name="Missions" />
         ...
         <div className="flex items-center gap-2">
          ...
          <div className="bg-gray-50 rounded-xl shadow-inner p-4">
            <h3 className="text-lg font-semibold text-blue-900 mb-6">Répartition des véhicules</h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={pieData}
